...
hikvision.on('alarm', function (code, action, index) {
	if (code === 'VideoMotion' && action === 'Start')
		if (motionenabled === true) client.publish('/ipcam/1/motion/status', 'ON') //console.log(getDateTime() + ' Channel ' + index + ': Video Motion Detected')
	if (code === 'VideoMotion' && action === 'Stop')
		if (motionenabled === true) client.publish('/ipcam/1/motion/status', 'OFF') //console.log(getDateTime() + ' Channel ' + index + ': Video Motion Ended')
